...
# define class for translated STS Benchmark (in Indonesian)
class STSBenchmarkEvalID(STSEval):
    def __init__(self, task_path, seed=1111):
        logging.debug('\n\n***** Transfer task : STSBenchmark*****\n\n')
        self.seed = seed
        self.samples = []
        dev = self.loadFile(task_path, 'validation')
        test = self.loadFile(task_path, 'test')
        self.datasets = ['dev', 'test']
        self.data = {'dev': dev, 'test': test}

    def loadFile(self, fpath, split):
        sick_data = {'X_A': [], 'X_B': [], 'y': []}
        dataset = load_dataset(fpath, split=split, trust_remote_code=True, token="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")
        for line in dataset:
            sick_data['X_A'].append(line['text_1'])
            sick_data['X_B'].append(line['text_2'])
            sick_data['y'].append(line['correlation'])
