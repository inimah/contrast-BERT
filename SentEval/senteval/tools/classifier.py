    ...
    def fscore(self, devX, devy):
        ...
        with torch.no_grad():
            for i in range(0, len(devX), self.batch_size):
                ...
                output = self.model(Xbatch)
                pred = output.data.max(1)[1]
                yhat = np.append(yhat, pred.cpu().numpy())
                ytrue = np.append(ytrue, ybatch.cpu().numpy())
            yhat = np.vstack(yhat)
            ytrue = np.vstack(ytrue)
            fscore = f1_met.compute(predictions=yhat, references=ytrue)
        return fscore
    
    
    def logitscore(self, devX):
        self.model.eval()
    ...
    def predict_proba(self, devX):
        ...
        with torch.no_grad():
            for i in range(0, len(devX), self.batch_size):
                ...
                #vals = F.softmax(self.model(Xbatch))
