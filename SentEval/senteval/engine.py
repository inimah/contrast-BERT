...
from senteval.binaryFake import FakeCLSDev, FakeCLSTest
from senteval.binaryFakeTrain import FakeCLSTrain
from senteval.snli import SNLIEval
from senteval.snliFakeNews import SNLIFakeNewsEval
from senteval.trec import TRECEval
from senteval.sick import SICKEntailmentEval, SICKEval
from senteval.mrpc import MRPCEval
from senteval.sts import STS12Eval, STS13Eval, STS14Eval, STS15Eval, STS16Eval, STSBenchmarkEval, SICKRelatednessEval, STSBenchmarkFinetune, STSBenchmarkEvalID
from senteval.stsFakeNews import FakePairEval
...
class SE(object):
    def __init__(self, args, params, batcher, prepare=None):
        ...
        self.list_tasks = ['CR', 'MR', 'MPQA', 'SUBJ', 'SST2', 'SST5', 'TREC', 'MRPC', 
                           'FakeCLSDev', 'FakeCLSTrain', 'FakePairs', 'FakePairsNLI', 'FakeCLSTest',
                           'SICKRelatedness', 'SICKEntailment', 'STSBenchmark',
                           'SNLI', 'ImageCaptionRetrieval', 'STS12', 'STS13',
                           'STS14', 'STS15', 'STS16',
                           'Length', 'WordContent', 'Depth', 'TopConstituents',
                           'BigramShift', 'Tense', 'SubjNumber', 'ObjNumber',
                           'OddManOut', 'CoordinationInversion', 'SICKRelatedness-finetune', 'STSBenchmark-finetune', 'STSBenchmark-fix', 'STSBenchmark-ID']
    ...
    def eval(self, name):
        ...
        # LazarusNLP/stsb_mt_id --> using translated (en - id) of STSBenchmark as evaluation set
        elif name == 'STSBenchmark-ID':
            self.evaluation = STSBenchmarkEvalID('LazarusNLP/stsb_mt_id', seed=self.params.seed)
        
        # for pairwise evaluation 
        elif name == 'FakePairs':
            self.evaluation = FakePairEval(self.args.pairsdatapath, seed=self.params.seed)
        # NLI based Fake News
        elif name == 'FakePairsNLI':
            self.evaluation = SNLIFakeNewsEval(self.args.pairsdatapath, seed=self.params.seed)
        elif name == 'FakeCLSTest':
            self.evaluation = FakeCLSTest(self.args.evalpath, self.args.eval_attribute, self.args.do_eval, seed=self.params.seed)
        elif name == 'FakeCLSDev':
            self.evaluation = FakeCLSDev(self.args.evalpath, self.args.eval_attribute, self.args.do_eval, seed=self.params.seed)
        elif name == 'FakeCLSTrain':
            self.evaluation = FakeCLSTrain(self.args.evalpath, self.args.eval_attribute, self.args.do_eval, seed=self.params.seed)
